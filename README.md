...
| `pytest -v` | Run the full test suite (393 tests, everything mocked, no network/credentials needed) |
...
This project spent a week (four focused passes, one per day) hardening the ADK agent graph against four distinct risk categories. This section is the quick, accurate summary; the [Security, by design](#security-by-design) table below has the field-by-field detail, and [`specs/write_action_gate_spec.md`](./specs/write_action_gate_spec.md) has the full investigation writeups (§1-8 write-action gate, §9 memory/dedup hardening, §10 agent-to-agent handoff hardening).
...
│
├── Tests
│   └── tests/                    # 393 tests, one file per module, all mocked
