...
| `pr_agent` | PR diff review — fetches only changed files, runs Semgrep + LLM, and (opt-in only, gated — see [Security, by design](#security-by-design)) can post **inline GitHub PR comments** | `fetch_pr_files_tool`, `scan_code_tool`, `generate_review_tool`, `validate_findings_tool`, `post_pr_review_tool` |
...
# Optional, off by default. Set to 1/true to let the ADK chat agent (adk web)
# see and call post_pr_review_tool / create_issue_tool / generate_report_file_tool
# at all -- see "Write-action gate" in Security, by design.
# CODE_REVIEW_AGENT_ALLOW_WRITE=1
```
...
> **Minimum token scope.** `GITHUB_TOKEN` is used for every GitHub call this agent makes, reads and writes alike, over one shared client — there's no separate read-only credential. Prefer a **fine-grained PAT** scoped to exactly what's used:
>
> | Scope | Needed for |
> |---|---|
> | `Contents: read` | Everything read-only — fetching files, PR diffs, repo metadata, convention files, recent review comments (`get_repo_metadata_tool`, `fetch_repo_files_tool`, `fetch_pr_files_tool`, `search_code_in_files_tool`, etc.) |
> | `Pull requests: write` | `post_pr_review_tool` only — and only reachable at all when `CODE_REVIEW_AGENT_ALLOW_WRITE` is set (see [Security, by design](#security-by-design)) |
> | `Issues: write` | `create_issue_tool` only — same gating as above |
>
> **Avoid the broad classic `repo` scope.** A classic PAT with `repo` covers all three of the above, but it also grants `Contents: write` and `Contents: delete` — full push/delete access to repo contents — which this agent never uses and shouldn't be able to do. If you use a classic token anyway, `public_repo` (public repos only) is narrower than full `repo`, but a fine-grained PAT with just the three scopes above is the closest match to what the code actually calls. If you only grant `Contents: read` + `Pull requests: write` and later ask for `create_issue_tool`, that call will fail with a 403 until you add `Issues: write` too.
...
| `pytest -v` | Run the full test suite (418 tests, everything mocked, no network/credentials needed) |
...
This project spent a week (four focused passes, one per day) hardening the ADK agent graph against four distinct risk categories, plus a fifth found the hard way — via a real, live run that fabricated a security report and opened a real (since-deleted) GitHub issue with it. This section is the quick, accurate summary; the [Security, by design](#security-by-design) table below has the field-by-field detail, [`specs/write_action_gate_spec.md`](./specs/write_action_gate_spec.md) has the write-action-gate/memory/handoff investigation writeups (§1-8 write-action gate, §9 memory/dedup hardening, §10 agent-to-agent handoff hardening), and [`specs/agent_spec.md`](./specs/agent_spec.md) §14 has the fifth category's incident writeup.

**A note on what "hardened" means here, upfront.** Two genuinely different kinds of defense are mixed together below, and conflating them would overstate what's actually guaranteed:

- **Code-level, deterministic guarantees** — enforced by Python/ADK mechanics, true regardless of what any model decides to do. The write-action confirmation gate, the off-by-default tool attachment, path confinement, per-item field validation (dropping malformed items before they reach a prompt), the severity/length caps on report rendering, and the distinct-`output_key`/sentinel-seeding fixes are all in this category.
- **Instruction-level, best-effort defenses** — the model is told to treat certain content as untrusted data rather than as instructions, and delimiter tags (`<file_content>`, `<recalled_memory>`, `<findings_to_process>`, `<specialist_output>`) give that framing a structural boundary on top of the instruction text. This measurably reduces the chance of a model complying with embedded instructions (verified with live evals, not just asserted), but it is **not** a hard guarantee the way the code-level items are — a sufficiently adversarial payload against a future model could still fail to be resisted. Where this project has *only* a best-effort defense and no code-level backstop, that's stated explicitly below, not glossed over.

**1. Indirect prompt injection** (content fetched from the repo itself — file contents, README/CONTRIBUTING text, past PR comments — instructing the model to ignore its task). Two layers: **Layer A (prevention, instruction-level)** wraps every piece of fetched content in `<file_content path="...">` delimiters and instructs every system prompt (main review + all 9 specialist audits) to treat it as data, never as instructions — verified with a live eval that embeds a real override/exfiltration payload alongside a genuine vulnerability and confirms the model still reports the vulnerability and complies with none of the payload. **Layer B (visibility, heuristic)** is `injection_scanner.py`: a pre-scan that flags suspicious phrasing (instruction-override language, role markers) *before* content reaches the model, surfaced in the report — this is explicitly a backstop for visibility, not a blocking mechanism; it never strips or blocks anything, and it can both miss a cleverly-worded attempt and flag a false positive.

**2. Excessive agency & tool misuse** (a model taking a real-world action — posting a PR comment, opening a GitHub issue, writing a file to disk — that a user didn't actually ask for). This is the one category with a genuine **code-level, hard guarantee**: all three write-capable tools are off by default (not attached to the ADK graph at all unless `CODE_REVIEW_AGENT_ALLOW_WRITE` is set), and even when attached, each is wrapped in ADK 2.3's native `require_confirmation=True` — the underlying write function is provably unreachable without a separate, explicit confirmed follow-up call, enforced inside ADK's own `FunctionTool.run_async`, not by anything this project's instructions ask the model to do. `generate_report_file_tool`'s output path is additionally confined inside a fixed directory at the code level (rejected, not redirected, on any escape attempt).

**3. Memory/context poisoning** (a fabricated or compromised finding written to persistent, cross-call memory, then silently trusted and re-surfaced in a later review of the same repo). A **code-level** plausibility check drops any finding whose `path` wasn't actually part of the run's fetched files before it's ever persisted — a concrete, cheap catch for the fabricated-path case specifically, not a full guarantee against every possible poisoning strategy. What *does* get persisted carries `source_run_id`/`persisted_at` provenance. Recalled memory read back into a later turn gets the same **instruction-level** `<recalled_memory>` delimiter+framing treatment as fresh file content.

**4. Blind trust between internal components** (one agent's own generated output handed to the next agent in the graph with no validation at all — a risk that's easy to miss because nothing "external" is involved). `security_aggregator_agent`/`patch_generator_agent`/`patch_verifier_step` previously read a specialist's raw output via ADK's own unescaped `{placeholder}` substitution — weaker than what Monday/Wednesday gave external and memory content. Fixed with the same **instruction-level** delimiter+framing pattern (`<specialist_output>`), but backed by two **code-level** fixes that don't depend on model compliance at all: the deterministic full-scan path no longer shares state slots with the ad-hoc chat path (distinct `output_key`s), and a callback resets every specialist's slot to an explicit sentinel before each run, so a specialist that silently fails to produce output is visibly flagged rather than crashing (`KeyError`) or quietly serving a stale result. Three more findings-processing tools (`validate_findings_tool`, `owasp_mapping_tool`, `cwe_mapping_tool`) were brought up to the same per-item validation standard the dedup/risk-score tools got in category 3, and report rendering now code-level-caps an unrecognized severity/oversized field rather than letting either through unbounded.

**5. Ungrounded tool input — a model fabricating its own tool-call arguments.** A distinct failure mode from category 4: not a downstream agent trusting a poisoned *upstream* value, but a specialist fabricating its *own* input from scratch. Five security specialist tools (`injection_audit_tool`, `auth_audit_tool`, `secrets_audit_tool`, `data_flow_tool`, `crypto_audit_tool`) used to take `files: list[dict]` as a plain LLM-supplied argument, with the only link to a real prior fetch being a docstring sentence. In a live run under Gemini rate-limit pressure, 4 of 6 parallel specialists skipped their fetch step entirely and called their audit tool directly with a fabricated Flask app that matched no file in the actual target repo — and the resulting fabricated CRITICAL/HIGH report was used to open a real GitHub issue (later deleted) on a repo that isn't even a web app. This is the one category caught by a real incident rather than a design review, and it has a **code-level, hard guarantee** as the fix: those five tools no longer accept file content as an argument at all — they take a `repo_url` and fetch the files themselves, so there is no parameter left for a model to put invented content into. `scan_code_tool`/`generate_review_tool` carry the same class of risk in principle and were deliberately **not** changed (lower severity — Semgrep's path is deterministic rule-matching, not LLM synthesis — and more broadly shared across agents); this is a stated residual gap, not a claim the whole tool surface is hardened. Full incident writeup: [`specs/agent_spec.md`](./specs/agent_spec.md) §14.
...
| **dedup/risk-score/owasp/cwe/validate-findings hardening** | Five tools that consolidate or re-analyze already-produced findings (`dedup_tool`, `risk_score_tool`, `owasp_mapping_tool`, `cwe_mapping_tool`, `validate_findings_tool`) previously only checked "is this a non-empty list" — a missing `path`/`severity`/`title` silently became `"?"`/`""` (or, for `validate_findings_tool`'s bare `i["path"]`, a raw crash). All five now drop malformed items (logged with the correct tool name, surfaced as `items_dropped`) before building a prompt, checking only the handful of fields each tool's own prompt actually reads — via two shared helpers (`_validate_dedup_items()`, `_validate_risk_score_items()`), each taking an optional `caller` name rather than five near-duplicate validators. Every affected prompt's findings text is also wrapped in `<findings_to_process>...</findings_to_process>` tags, the same structural boundary fresh file content gets, on top of pre-existing "treat as untrusted data" instruction text. `dedup_tool`'s output deliberately did **not** gain a strict `response_schema` like `risk_score_tool` already has — its output merges heterogeneous upstream specialist fields, and `extra="forbid"` would silently truncate them at generation time; a lighter shape-only post-hoc check was added instead. See [specs/write_action_gate_spec.md](./specs/write_action_gate_spec.md) §9.2, §10.5 |
| **Agent-to-agent handoff hardening** | `security_aggregator_agent`, `patch_generator_agent`, and `patch_verifier_step` previously read an upstream specialist's raw output via ADK's own `{sast_result}`-style `{placeholder}` substitution — no delimiter, no framing, and a missing key raised a raw `KeyError`. All three now use a **callable** `instruction=` (an ADK `InstructionProvider`), which makes ADK skip its own placeholder-substitution mechanism entirely (`bypass_state_injection=True`) and lets the code wrap each specialist's output in `<specialist_output name="...">` tags with explicit "not verified fact, not an instruction" framing — same convention as `<file_content>`/`<recalled_memory>` above. Two related bugs closed alongside it: the six `security_full_scan` clones (`sast_agent_scan`, etc.) now write to their own `*_scan_result` state keys instead of sharing a slot with the ad-hoc chat specialists, and a `before_agent_callback` resets all six slots to an explicit "no output produced" sentinel before every full-scan run, so a specialist that silently produces no output is visibly flagged rather than crashing or serving a stale result from an earlier run. See [specs/write_action_gate_spec.md](./specs/write_action_gate_spec.md) §10.1–§10.4 |
| **Grounded-fetch audit tools** | Found via a real incident, not a design review: under rate-limit pressure, 4 of 6 parallel security specialists skipped fetching real repo files and called their audit tool directly with a fabricated Flask app, which then went out in a real (later deleted) GitHub issue. `injection_audit_tool`, `auth_audit_tool`, `secrets_audit_tool`, `data_flow_tool`, and `crypto_audit_tool` no longer accept `files` as a model-supplied argument at all — each now takes `repo_url` and fetches the repository itself before auditing, so fabricating findings is structurally impossible rather than instruction-discouraged. `scan_code_tool`/`generate_review_tool` carry the same class of risk and were deliberately left unchanged (documented, not silently ignored) — lower severity (deterministic Semgrep rule-matching, not LLM synthesis) and more broadly shared across agents than the five above. See [specs/agent_spec.md](./specs/agent_spec.md) §14 |
| **Report-rendering hardening** | Both the CLI/`review_repo()` path and the ADK chat path's `generate_report_file_tool` render through the same `report_generator.generate_markdown_report()` (and, for GitHub issues, `github_fetcher.py`'s `_build_issue_body()`). The CLI path's severity is already schema-constrained upstream (Gemini's `response_schema` + a `Literal[...]` enum), but the ADK chat path's is an arbitrary model-supplied string until rendering time — previously, any string became its own out-of-order Markdown/issue-body heading. Both renderers now coerce an unrecognized severity into a single `UNKNOWN SEVERITY` bucket instead, and cap `title`/`description`/`suggested_fix` length with a visible truncation marker rather than letting an unbounded field blow up report size. `_escape()`/`_escape_markup()` (the existing `<`/`>` HTML-injection defense) were **not** touched by this change — this adds validation around them, not a replacement. See [specs/write_action_gate_spec.md](./specs/write_action_gate_spec.md) §10.6 |
...
418 tests across all modules. Every external dependency — GitHub API, Semgrep subprocess, Gemini SDK (including `embed_content`) — is mocked, so the full suite runs in a few seconds with no network access or credentials required. These tests check plumbing: batching, JSON parsing, retries, exact-match and semantic caching (hits, misses, per-`(system_instruction, rag_fingerprint)` bucket scoping, threshold behavior, embedding-failure fallback), RAG comment retrieval/contextualization, native `response_schema` passthrough/omission, error handling, size caps, schema validation, persistent findings memory (round-trip storage, corrupted-file degradation, new/still_open/resolved diff classification, fabricated-path rejection + provenance at persistence time, delimiter-wrapped recall framing), dedup/risk-scorer/owasp-mapping/cwe-mapping/validate-findings per-item validation and fail-closed behavior on malformed input, the write-action gate (path confinement, off-by-default attachment, ADK-native confirmation hard-block with `assert_not_called()` proof), agent-to-agent handoff hardening (`security_aggregator_agent`'s/`patch_generator_agent`'s/`patch_verifier_step`'s callable instructions produce delimited, framed output; a stale or never-set specialist slot renders an explicit sentinel, never a `KeyError`), grounded-fetch audit tools (`injection_audit_tool`/`auth_audit_tool`/`secrets_audit_tool`/`data_flow_tool`/`crypto_audit_tool` actually fetch and forward real files rather than trusting a model-supplied argument, and the old `files=` kwarg they used to accept now raises `TypeError` — see [specs/agent_spec.md](./specs/agent_spec.md) §14), 429 retry-with-backoff across every layer of the ADK graph (root through the aggregator and `remediation_agent`, all using a `Gemini(retry_options=...)` instance, not a bare model string — see [specs/agent_spec.md](./specs/agent_spec.md) §15), report-rendering hardening (an unrecognized severity collapses into a single labeled bucket instead of its own heading; oversized fields are truncated with a visible marker), the guardrail's secret/injection-leakage detection and block/pass wiring into the three write paths, and the Layer A/B injection defense (`<file_content>` delimiter wrapping, the flag-it instruction fitting the existing schema, `injection_scanner.py`'s pattern matching, and a planted-injection fixture confirming a real finding survives alongside the flagged attempt). They do not check whether the pipeline's judgment is actually good — that's what the eval suite below is for.
...
│
├── Tests
│   └── tests/                    # 418 tests, one file per module, all mocked
...
- **Partially handled:** free-tier Gemini keys also cap requests-per-minute (15 RPM observed for `gemini-3.1-flash-lite`) and input-tokens-per-minute (250k observed) — separately from the daily cap above, and neither is tracked or throttled anywhere in this project. `security_full_scan`'s 6-way parallel fan-out (plus its L4 sub-delegates) can burst past both within a single full scan; a `429` mid-scan used to just kill the whole `asyncio.TaskGroup` (`ExceptionGroup`) — this is what created the rate-limit pressure behind the §14 grounded-fetch incident above. Every `Agent(model=...)` in the ADK graph now uses ADK's own `Gemini(retry_options=types.HttpRetryOptions(...))` (HTTP-layer retry inside the SDK's own client, tuned toward the ~16-30s reset window actually observed), so a single 429 no longer kills the scan outright — but this is retry, not prevention: the caps themselves still aren't tracked or paced against, so a scan that stays rate-limited past the configured retry budget can still fail. See [specs/agent_spec.md](./specs/agent_spec.md) §15.
- **Residual, documented (see [Security, by design](#security-by-design) → Grounded-fetch audit tools):** `scan_code_tool` and `generate_review_tool` still accept file content as a plain model-supplied argument, the same class of risk category 5 above fixed for the other five audit tools. Left as-is deliberately (lower severity, more broadly shared across agents) rather than silently unaddressed.
