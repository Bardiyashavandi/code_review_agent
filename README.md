...
| `pytest -v` | Run the full test suite (409 tests, everything mocked, no network/credentials needed) |
...
│
├── Tests
│   └── tests/                    # 409 tests, one file per module, all mocked
...
- **Not yet handled:** free-tier Gemini keys also cap requests-per-minute (15 RPM observed for `gemini-3.1-flash-lite`) and input-tokens-per-minute (250k observed) — separately from the daily cap above, and neither is tracked or throttled anywhere in this project. `security_full_scan`'s 6-way parallel fan-out (plus its L4 sub-delegates) can burst past both within a single full scan, and a `429` mid-scan currently just kills the whole `asyncio.TaskGroup` (`ExceptionGroup`) rather than backing off and retrying — this is what created the rate-limit pressure behind the §14 grounded-fetch incident above. Retry-with-backoff for the ADK graph's own model calls is the same "not yet handled" gap noted two bullets up (no fallback/caching/routing in the ADK graph), not a new one.
