...
A root orchestrator agent (`code_review_agent`) delegates to a **five-layer, 37-LLM-agent pipeline**, with two paths built on ADK's deterministic workflow-agent primitives (`ParallelAgent`, `SequentialAgent`, `LoopAgent`) instead of LLM-driven delegation — see "Deterministic workflow-agent patterns" below:
...
3. **Security analysis.** `security_coordinator` orchestrates six specialist agents: `sast_agent` (Semgrep + LLM), `injection_agent` (SQL/cmd/SSTI/XSS/SSRF/path traversal), `auth_agent` (IDOR/broken auth/privilege escalation), `crypto_agent` (weak algorithms), `secrets_agent` (hardcoded credentials), and `data_flow_agent` (taint analysis: input → dangerous sink). `validator_agent` cross-checks SAST findings; `taint_validator_agent` confirms path reachability. A "full/comprehensive security review" request routes to `security_full_scan` instead — a `ParallelAgent` running all six specialists concurrently followed by `security_aggregator_agent`, deterministically guaranteeing none get silently skipped, rather than hoping the LLM remembers to call all six itself.
4. **Quality analysis.** `quality_coordinator` orchestrates four specialists: `quality_agent` (best practices, RAG-grounded in the repo's own README/CONTRIBUTING/lint config and past PR review comments so findings cite the project's own conventions rather than only generic advice), `complexity_agent` (cyclomatic complexity, god classes), `test_agent` (coverage gaps), and `doc_agent` (missing docstrings, type hints).
...
6. **Consolidation.** `dedup_agent` merges cross-agent duplicate findings; `risk_scorer_agent` assigns CVSS-like composite risk scores; `remediation_agent` generates copy-pasteable before/after code patches — and is itself a verify-and-refine `LoopAgent` (`patch_generator_agent` → `patch_verifier_step`, up to 3 iterations) that re-checks whether a generated patch actually resolves its finding before presenting it, rather than trusting a single-shot guess.
...
L1  planner_agent ── context_agent ── scout_agent ── pr_agent
    ...
    remediation_agent (LoopAgent: patch_generator_agent ⇄ patch_verifier_step, max 3x)
 │
L2  security_coordinator ── quality_coordinator ── intel_coordinator
 │           │                    │                      │
 │  security_full_scan (SequentialAgent: ParallelAgent[6 specialists] → aggregator)
...
**Multi-agent system (ADK).** Thirty-seven LLM agents across five layers, built with Google ADK 2.3. The system uses ADK's `sub_agents` mechanism throughout: `planner_agent` delegates to three domain coordinators, each coordinator delegates to specialist agents, and several specialists (`sast_agent`, `data_flow_agent`, `compliance_agent`) delegate further to sub-specialists. Every transfer is explicit — no manual intent parsing, no procedural dispatch. The root agent exposes a `FunctionTool` (`review_repo_tool`) for one-shot fast reviews, while the full 5-layer hierarchy handles deep, targeted analysis. Agents cover injection detection (SQL/cmd/SSTI/XSS/SSRF), auth auditing (IDOR/privilege escalation), secrets scanning, taint analysis, cyclomatic complexity, test coverage gaps, documentation quality, OWASP Top 10 2021 mapping, CWE Top 25 mapping, cross-agent deduplication, CVSS-like risk scoring, and automated remediation patch generation.

**Deterministic workflow-agent patterns, not just LLM-driven delegation.** Every agent above relies on `transfer_to_agent` — an LLM deciding whom to hand off to. Two paths don't need that judgment call, they need a guarantee, so they're built on ADK's `ParallelAgent`/`SequentialAgent`/`LoopAgent` primitives instead: `security_full_scan` runs all six security specialists concurrently and deterministically aggregates their results, so a "full review" can never silently skip a specialist the way an LLM-driven sequence could. `remediation_agent` is a verify-and-refine `LoopAgent` — generate a patch, check whether it actually resolves the finding (re-running Semgrep, or a lightweight LLM-judged check for non-Semgrep findings), retry with that feedback up to 3 times, exit the moment it verifies clean. Both patterns extend past the ADK graph itself: `POST /remediate` and the Streamlit fix-generation button call `CodeReviewAgent` directly (bypassing the agent graph), so they use a parallel, non-ADK implementation of the same verify-and-refine shape (`generate_remediation_patches_with_verification`) to get the same behavior.

**Retrieval-augmented review (RAG).** Before reviewing, the pipeline indexes a repo's own conventions — README, CONTRIBUTING, lint config — plus its recent PR review comments, so findings can cite the project's *own* stated conventions rather than only generic best practices ("this violates this repo's own naming convention" vs. "consider PEP 8"). Past PR comments are embedded once with `gemini-embedding-001` (the same model and client already powering the semantic response cache — no new dependency) using an asymmetric retrieval task-type pairing (`RETRIEVAL_DOCUMENT` for indexing, `RETRIEVAL_QUERY` per batch), distinct from the cache's own `SEMANTIC_SIMILARITY` comparison use case. Indexed once per `(repo, branch)` and cached for the process's lifetime, since conventions don't change per-file or per-review; every step degrades gracefully to an empty context (not a failed review) if a repo has no conventions doc or PR history.
...
A capstone project that only ever sees mocked inputs proves the code parses correctly, not that it works. So beyond the 245-test mocked suite (covering batching logic, severity sorting, error handling, input/output validation, exact-match and semantic caching, the `/remediate` HTTP route, RAG project-context indexing/retrieval, the deterministic `security_full_scan`/`remediation_agent` workflow constructions, and the security cases above — all running in a few seconds with no network access or credentials) and a separate 23-case scenario-based eval suite that scores the pipeline's actual judgment against real Gemini calls (detection accuracy, false-positive rate, dedup effectiveness, risk-scoring correctness, resistance to an embedded prompt-injection attack, full-security-scan coverage, and remediation-loop convergence — see `evals/README.md`), this project was run end-to-end against a real, unmodified GitHub repository with real credentials, real network calls, and real LLM output.
...
The project grew from a single-agent pipeline into a thirty-seven-LLM-agent, five-layer system with two deterministic workflow-agent paths layered on top — not to check rubric boxes, but because the multi-agent design naturally maps to how a real security team would divide the work: a strategist to plan, domain leads to coordinate, specialist analysts for each attack class, sub-specialists to validate and map findings to standards, cross-cutting agents to deduplicate and score risk, and — where the task genuinely calls for a guarantee rather than a judgment call — deterministic parallel execution and verify-and-refine loops instead of LLM-driven delegation. The 245 tests, the 23-case eval suite, the CI pipeline, the tracing endpoint, and the three real bugs found during end-to-end testing are the evidence that this is a working system, not a demo assembled for a deadline.
