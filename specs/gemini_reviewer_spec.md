...
- [x] All tests in `tests/test_gemini_reviewer.py` (including `TestResponseSchema`) pass with `pytest -v`
- [x] Every `_call_model` call site NOT listed in §12.2 passes no `response_schema` and behaves identically to before this change
- [x] `_parse_response`'s post-hoc Pydantic validation of `_ReviewResponseSchema` is unchanged and still runs on every batch
...
### 12.7 Real-API bug found and fixed — `additionalProperties` rejected outside Vertex AI

The plumbing in §12.1-12.4 was fully covered by mocked unit tests but never actually exercised against the real Gemini API until a live Streamlit run (§12's first live use since this feature was added). That run failed the review stage entirely — `Gemini API error 400 ... Invalid JSON payload received. Unknown name "additional_properties" at 'generation_config.response_schema' ... Fallback to gemini-2.5-flash-lite also failed` — because every schema passed as `response_schema` (`_ReviewResponseSchema`, `_IssueSchema` nested inside it, and the same for `_RemediationResponseSchema`/`_PatchSchema`, `_RiskScoreResponseSchema`/`_ScoredFindingSchema`, `_PatchVerificationSchema`) uses `model_config = ConfigDict(extra="forbid")`, which Pydantic's `.model_json_schema()` renders as `additionalProperties: false`. Confirmed by reading the installed `google-genai==2.9.0` SDK's own `_transformers._raise_for_unsupported_mldev_properties`: `additionalProperties` is explicitly rejected for the free-tier Gemini Developer API (only supported in Vertex AI's "Enterprise Agent Platform mode"). The mocked test suite could never surface this — it mocks `GeminiReviewer`/`_call_model` (or the `genai.Client` itself) at every call site, so the actual request never reaches the SDK's serialization/validation path.

**Fix, not a downgrade:** `_call_model` no longer sets `GenerateContentConfig.response_schema` to the raw Pydantic class. It instead calls `response_schema.model_json_schema()` and passes the resulting plain dict through `GenerateContentConfig.response_json_schema` — a separate, SDK-documented field whose supported-property list explicitly includes `additionalProperties`, `$defs`, and `$ref` (needed for the nested-model case, e.g. `_ReviewResponseSchema.issues: list[_IssueSchema]`). This was deliberately NOT fixed by loosening `extra="forbid"` on these classes: they're dual-purposed — also used for this file's existing strict post-hoc validation (`_parse_response()` etc., a documented Security-by-design guarantee) — and weakening that would have been a real regression traded for a generation-time fix that didn't need it.

Verified: SDK source inspection (as above) plus a direct, unmocked call in a Python REPL confirming `response_schema.model_json_schema()` on `_ReviewResponseSchema` does contain a nested `additionalProperties: false` (i.e. reproduces the exact shape that trips the SDK's guard). A live end-to-end call against the real API to confirm the *fix* itself wasn't possible from the environment this fix was written in (network policy blocks outbound Gemini API calls there) — confirm with a real `streamlit run` / CLI review against a live `GEMINI_API_KEY`.

`tests/test_gemini_reviewer.py::TestResponseSchema` updated: every assertion that checked `config.response_schema is <SchemaClass>` now checks `config.response_json_schema == <SchemaClass>.model_json_schema()` and separately asserts `config.response_schema is None`, plus one new regression test (`test_response_json_schema_contains_no_additional_properties_key_error_regression`) asserting the schema actually placed on the wire is a plain dict, not a value that would route back through `response_schema`'s rejecting conversion path.
